<!--
  Canonical design reference for seedit: use this for colors, type, spacing, motion,
  components, and accessibility decisions unless the user specifies otherwise.
  Upstream: https://github.com/Khalidabdi1/design-ai/blob/main/design-md/x/DESIGN.md
  License follows the design-ai repository.
-->
...
| X Blue | `#1D9BF0` | rgb(29, 155, 240) | Links, primary actions — **not** native text selection or keyboard focus rings (see [seedit: text selection](#seedit-text-selection-do-not-change) and [seedit: focus rings](#seedit-focus-rings-do-not-change) below) |
...
### seedit: text selection (do not change)

**Policy (seedit):** Browser default and X-style **blue** text selection are **not** used. Native `::selection` is **monochrome** so it stays distinct from link blue and matches the feed’s high-contrast look.

| Theme | Selection background | Selection text |
|-------|----------------------|----------------|
| Light | `#000000` (black) | `#FFFFFF` (white) |
| Dark | `#FFFFFF` (white) | `#000000` (black) |

**Implementation:** CSS variables `--shadcn-selection` and `--shadcn-selection-foreground` (HSL triplets) in `src/styles/shadcn.css`, applied in `@layer base` for `::selection` / `::-moz-selection`.

**Do not change** this to X Blue, system accent, or other colors without an explicit product decision; update this section if the policy changes.

### seedit: focus rings (do not change)

**Policy (seedit):** Keyboard focus outlines and component `ring` tokens use the **same monochrome pairing** as text selection: **black on light**, **white on dark**, so focus stays high-contrast and distinct from X Blue links and primary buttons.

| Theme | Focus ring (`--shadcn-ring`) |
|-------|------------------------------|
| Light | `#000000` (black) |
| Dark | `#FFFFFF` (white) |

**Implementation:** `--shadcn-ring` (HSL triplet) in `src/styles/shadcn.css` under `.light` / `.dark`; Tailwind `ring-ring` and any `hsl(var(--shadcn-ring))` outlines pick this up.

**Do not change** focus rings to X Blue without an explicit product decision; update this section if the policy changes.
...
.tab-item.active {
  color: #E7E9EA;
  font-weight: 700;
  /* seedit: active tab underline uses foreground (same as text), not X Blue */
...
/* Focus rings — seedit uses monochrome ring via --shadcn-ring (black light / white dark) */
.focus-ring {
  box-shadow: 0 0 0 2px hsl(var(--shadcn-ring));
}
...
- Inputs: Minimal borders; focus ring black (light) / white (dark), not X Blue
...
- Focus: Monochrome ring by theme (see seedit: focus rings)
- Disabled: 50% opacity
```
